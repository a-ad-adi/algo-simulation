...
class SortSimulation extends Component {
  snapshot;
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidMount(){
    ...
    const steps = [<Step actions={this.state.showControls} key={this.state.stepNo} hasNext={true} next={this.getNext}/>];
    this.setState({steps});    
  }
  ...
  getNext(){
    ...
    if(this.state.stepNo < this.snapshot.steps.length){
      ...
      steps.push(<Step actions={this.state.showControls} key={this.state.stepNo} hasNext={true} next={this.getNext}/>);      
      this.setState({steps});      
    }else{
      console.log("Notify")
    }
